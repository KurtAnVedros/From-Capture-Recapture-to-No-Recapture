...
def mapCodeSignal(Signal, Code, padding, startCycleCut = 0, labels=True, skip_start_peaks=0):
    cycleOn = 0
    ...
    listCodes.append(["adc", 1, "silver", 0])
    ...
    listCodes.append(["brcc-f", 1, "darkorange", 0])
    listCodes.append(["brcc-t", 2, "burlywood", 0])
    ...
    listCodes.append(["brlo-f", 1, "navajowhite", 0])
    listCodes.append(["brlo-t", 2, "orange", 0])
    ...
    listCodes.append(["cpi", 1, "royalblue", 0])
                    ...
                    if listCodes[codeNum][3] == 0:
                        if cycleOn == 0:
                            mapGraphTool(Signal, padding, cycleOn, listCodes[codeNum][1]-startCycleCut, listCodes[codeNum][0], 
                                 listCodes[codeNum][2],  True, skip_start_peaks=skip_start_peaks)
                        else:
                        ...
                        mapGraphTool(Signal, padding, cycleOn, listCodes[codeNum][1], listCodes[codeNum][0], 
                                 listCodes[codeNum][2], skip_start_peaks=skip_start_peaks)
                    if cycleOn == 0:
...
def mapGraphTool(Signal, padding, cycleStart, Cycles, name, cName, ifLabel=False, skip_start_peaks=0):
    
    peakLocStart = cycleStart * 2 + skip_start_peaks
    if cycleStart == 0:
        seqStart = 0 + getPeaksLoc(Signal, padding, skip_start_peaks)
    else:
...
def getPeaksDataset(dataset, pad, getHigh = True):
    dataset_Peaks = []

    for signal in dataset:
        temp = getPeaks(signal, pad, getHigh = getHigh)
...
'''
Graphs the mapping on a signal.
'''
def MapGraph(Signal, code, pad, y_low=0, y_high=0, save_info=""):
    time_Signal = np.arange(Signal.shape[0])
    plt.plot(time_Signal, Signal)
    mapCodeSignal(Signal, code, pad)
    ...
    if (save_info != ""):
        plt.savefig(save_info, bbox_inches='tight')
    plt.tick_params(direction='in', length=30, width=3, grid_alpha=0.5)
    plt.show()
    plt.close()
...
def print_MapGraph_Code():
    print('time_Signal = np.arange(Signal.shape[0])')
    print('plt.plot(time_Signal, Signal)')
    print('mapCodeSignal(Signal, code, pad)')
    print('plt.xlabel("Sample Index")')
    print('plt.ylabel("Amplitude")')
    print('plt.grid(axis="y")')
    print('')
    print('plt.axvspan(MapTool.getPeaksLoc(Signal, pad, 24, False), MapTool.getPeaksLoc(Signal, pad, 268), color="blue", alpha=0.1)')
    print('plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.12), fancybox=True, shadow=True, ncol=6)')
    print('plt.xlim([0,len(Signal)])')
    print('plt.ylim(y_low,y_high)')
    print('plt.savefig(save_info, bbox_inches="tight")')
    print('plt.tick_params(direction="in", length=30, width=3, grid_alpha=0.5)')
    print('plt.show()')
    print('plt.close()')

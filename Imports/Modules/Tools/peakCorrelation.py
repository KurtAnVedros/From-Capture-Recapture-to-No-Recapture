...
'''
def getPercentageFidelity(real_CyclesCCs, gen_CyclesCCs, path, printIt=False, Graph=True, y_min=80, y_max=100, real_gen_Sections = False, real_gen_Cycle = 5, xy_tick_fontsize=12):
    percentage_Cycles = []
    ...
    if Graph:
        if real_gen_Sections:
            # Sample data: a list of floats between 0 and 1
            data = percentage_Cycles

            # Create x positions for each bar based on the list indices
            ...
            plt.xlabel('Cycle')
            plt.ylabel('Percent')
            #plt.title('Bar Graph for Each Instance in the List')

            # Optionally set x-axis ticks to be each index value
            plt.xticks(indices, fontsize=xy_tick_fontsize)
            plt.yticks(fontsize=50)

            plt.ylim(0, y_max)
            plt.grid(axis='y')

            disect_loc = real_gen_Cycle - 0.5
            # Add a vertical line between instance 5 and 6 (at x = 5.5)
            plt.axvline(x=disect_loc, color='blue', linestyle='--', linewidth=7)

            # Determine a y-position for text annotations near the top (5% below y_high)
            y_text = 0 + (y_max - 0) * 0.02

            # Add text labels near each vertical line
            plt.text(disect_loc, y_text, "< Real ", color='b', ha='right', va='bottom', fontsize=50)
            plt.text(disect_loc, y_text, " Generated >", color='g', ha='left', va='bottom', fontsize=50)

            # Display the plot
            plt.savefig(path+ "Graphs/Cycle_Percentage.png", bbox_inches='tight')
            plt.show()
            plt.close()
            ...
            # Display the plot
            plt.savefig(path+ "Graphs/Cycle_Percentage_zoom.png", bbox_inches='tight')
            plt.show()
            plt.close()
...
RETURNS:
    aroundPeak_Dataset: dataset of signals containing only the peaks and around the peak information.
'''
def getCenters(dataset, pad, if_MajorOnly=False, clean_dataset=0, use_Clean=False, printTime=False):
    start_time = time.time()
    peaks = MapTool.getPeaksDataset(dataset, pad)
...
def obtainSignalCentersDataset_Clean(signals, signals_clean, pad, amount, if_MajorOnly=True, midpoint=0, mini_len=5):
    new_Signals = []
    centerLocs_Set = getCenterLocsDataset(signals_clean, pad, if_MajorOnly, midpoint, mini_len)
...
RETURNS:
    aroundPeak_Dataset: dataset of signals containing only the peaks and around the peak information.
'''
def getCenterLocsDataset(dataset, pad, if_MajorOnly=True, midpoint=0, mini_len=5, printTime=False):
    start_time = time.time()
...
'''
def EuclideanByCenter(TS_Signals, Q_Signals, TS_Signals_Name, Q_Signals_Name, pad, numEucs, path, amountAround, if_MajorOnly=True, Graph=True, printIt=True, y_low=-2, y_high=2, y_low_Center=0.5, y_high_Center=1.5,  mini_len=5, midpoint=0, save=True, real_gen_Sections=False, real_gen_Peak=30):
    start_time = time.time()
    ...
    if Graph:
        if real_gen_Sections:
            # Graph the full signal
            Signal = TS_Signals[0]
            time_Signal = np.arange(Signal.shape[0])
            plt.plot(time_Signal, Signal, label = TS_Signals_Name)
            
            # Calculate positions for the vertical lines where it changes from real to generated signasls
            peak_change = MapTool.getPeaksLoc(Signal, pad, real_gen_Peak)-5
            
            Signal_2 = Q_Signals[0][peak_change:]
            time_Signal_2 = np.arange(peak_change, Q_Signals[0].shape[0])
            plt.plot(time_Signal_2, Signal_2, label = Q_Signals_Name)
            #plt.xlabel("Sample Index")
            plt.ylabel("Amplitude")
            plt.grid(axis='y')
            
            plt.axvspan(peak_change, len(Signal)+50, color='green', alpha=0.05)

            # Draw vertical lines without legends
            plt.axvline(x=peak_change, color='b', linestyle='--', linewidth=7)

            # Determine a y-position for text annotations near the top (5% below y_high)
            y_text = (y_high - (y_high - y_low) * 0.1)

            # Add text labels near each vertical line
            plt.text(peak_change, y_text, "< Real ", color='b', ha='right', va='bottom', fontsize=50)
            plt.text(peak_change, y_text, " Generated >", color='g', ha='left', va='bottom', fontsize=50)
            ...
            # Graph just the generated portion.
            # Calculate positions for the vertical lines where it changes from real to generated signasls
            peak_change = MapTool.getPeaksLoc(TS_Signals[0], pad, real_gen_Peak)-5
            
            Signal = TS_Signals[0][peak_change:]
            time_Signal = np.arange(peak_change, TS_Signals[0].shape[0])
            plt.plot(time_Signal, Signal, label = TS_Signals_Name)
            ...
            plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
                      fancybox=True, shadow=True, ncol=6)
            plt.xlim([peak_change,len(TS_Signals[0])])
            plt.ylim(y_low,y_high)
            plt.savefig(path+ "Graphs/Compare_FullSignal_GenPortionOnly.png", bbox_inches='tight')
            plt.show()
            plt.close()
            ...
            # Calculate positions for the vertical lines where it changes from real to generated signasls
            peak_change = int((real_gen_Peak/2) * (1+(amountAround * 2)))
            
            Signal_2 = signal_2[peak_change:]
            time_Signal_2 = np.arange(peak_change,len(signal_2))
            plt.plot(time_Signal_2, Signal_2, label = Q_Signals_Name)
            #plt.xlabel("Sample Index")
            plt.ylabel("Amplitude")
            plt.grid(axis='y')
            
            plt.axvspan(peak_change, len(Signal)+1, color='green', alpha=0.05)

            # Draw vertical lines without legends
            plt.axvline(x=peak_change, color='b', linestyle='--', linewidth=7)

            # Determine a y-position for text annotations near the top (5% below y_high)
            y_text = (y_high_Center - (y_high_Center - y_low_Center) * 0.1)
            ...
            plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
                      fancybox=True, shadow=True, ncol=6)
            plt.xlim([0,len(Signal)])
            plt.ylim(y_low_Center,y_high_Center)
            plt.savefig(path+ "Graphs/Compare_AroundCentersSignal.png", bbox_inches='tight')
            plt.show()
            plt.close()
            
            # Graph the centers graph just the generated
            signal_1 = obtainSignalCenters(TS_Signals[0], TS_Signals_centerLocs[0], amountAround)
            signal_2 = obtainSignalCenters(Q_Signals[0], Q_Signals_centerLocs[0], amountAround)
            ...
            Signal = signal_1[peak_change:]
            time_Signal = np.arange(peak_change,len(signal_1))
            plt.plot(time_Signal, Signal, label = TS_Signals_Name)
            ...
            plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
                      fancybox=True, shadow=True, ncol=6)
            plt.xlim([peak_change,len(signal_1)])
            plt.ylim(y_low_Center,y_high_Center)
            plt.savefig(path+ "Graphs/Compare_AroundCentersSignal_GenPortionOnly.png", bbox_inches='tight')
            plt.show()
            plt.close()
    ...
    # Obtain the comparison across the whole signal
    ## note: Due to the square root system, adding the euc across the cycles will not equal the euc across full signals. 
    avgDist, Dists = EuclideanByCenter_FullSignals(TS_Signals, Q_Signals, pad, numEucs, amountAround, if_MajorOnly=if_MajorOnly, show_time=False)
    ...
    if save:
        ...
        # Save arrays.
        np.save(path + "Arrays/Full_Signal_Dists", Dists)
    ...
    return Dists, np.array(AroundCenters_Euc_Total), getTotalCC(AroundCenters_Euc_Total)
...
RETURNS:
    ...
    AroundCenters_Euc_Total: lists of each signals avg euclean for each area around peaks added to each other.
'''
def EuclideanByCenter_FullSignals(TS_Signals, Q_Signals, pad, numEucs, amountAround, if_MajorOnly=True, show_time=False):
    start_time = time.time()
    
    # check if already around peak data and if not get it.
    TS_Signals_AroundCenters = obtainSignalCentersDataset(TS_Signals, pad, amountAround, if_MajorOnly=True, midpoint=0, mini_len=5)
    Q_Signals_AroundCenters = obtainSignalCentersDataset(Q_Signals, pad, amountAround, if_MajorOnly=True, midpoint=0, mini_len=5)
    
    Q_Signals_AroundCenters = Q_Signals_AroundCenters[:,:TS_Signals_AroundCenters.shape[1]]
    ...
    Dists = np.array(Euc_for_AroundCenter)
